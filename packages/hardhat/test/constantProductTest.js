    ...
    describe("ethToToken()", function () {
      ...
      it("Should transfer tokens to purchaser after trade", async function () {
        ...
        console.log(
          "\t",
          " 💵 User3 initial $BAL balance:",
          formatEther(user3_token_before)
        );
    ...
    describe("tokenToEth()", async () => {
      it("Should send 1 $BAL to DEX in exchange for _ $ETH", async function () {
        ...
        console.log(
          ...
          formatEther(balloons_bal_start)
        );
        const dex_eth_start = await ethers.provider.getBalance(
          dexContract.address
        );
      ...
      it("Should revert if 0 tokens sent to the DEX", async function () {
        await expect(dexContract.tokenToEth(ethers.utils.parseEther("0"))).to.be
          .reverted;
      });
      ...
      it("Should send less eth after the first trade (tokenToEth() called)", async function () {
        ...
        console.log(
          "\t",
          " 💵 Next Dex balance:",
          formatEther(dex_eth_next),
          " Eth sent:",
          formatEther(tx1difference.mul(-1))
        );
        ...
        console.log(
          ...
          formatEther(dex_eth_end),
          " Eth sent:",
          formatEther(tx2difference.mul(-1))
        );
    ...
    describe("withdraw()", async () => {
      ...
      it("Should withdraw 1 ETH and 1 $BAL when pool is at a 1:1 ratio", async function () {
        ...
        console.log(
          ...
          formatEther(userBallonsBalance)
        );
        ...
        console.log(
          ...
          formatEther(userBallonsBalanceAfter)
        );
        console.log(
          "\t",
      ...
      it("Should revert if sender does not have enough liqudity", async function () {
        ...
        await expect(dexContract.withdraw(ethers.utils.parseEther("100"))).to.be
          .reverted;
      });
      ...
      it("Should decrease total liquidity", async function () {
        ...
        const txWithdraw = await dexContract.withdraw(
          ethers.utils.parseEther("1")
        );
        ...
        console.log(
          ...
          formatEther(liquidityBurned)
        );

        expect(
          totalLpAfter,
          "Emitted incorrect liquidity amount burned"
        ).to.be.equal(totalLpBefore.sub(liquidityBurned));
        expect(
          totalLpBefore,
