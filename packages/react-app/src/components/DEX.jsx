    ...
    display.push(
      <div>
        ...
        {rowForm("deposit", "📥", async value => {
          ...
          let tokensDeposited = valueInEther.mul(tokenBalance).div(contractBalance).add(1);
          let allowance = await props.readContracts[tokenName].allowance(
            props.address,
            props.readContracts[contractName].address,
          );
          console.log("allowance", allowance.toString());
          if (allowance.lt(tokensDeposited)) {
            await tx(
              writeContracts[tokenName].approve(props.readContracts[contractName].address, tokensDeposited, {
                gasLimit: 200000,
              }),
            );
          }
